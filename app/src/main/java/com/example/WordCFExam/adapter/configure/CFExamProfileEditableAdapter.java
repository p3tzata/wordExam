package com.example.WordCFExam.adapter.configure;

import com.example.WordCFExam.activity.base.BaseEditableAppCompatActivity;
import com.example.WordCFExam.adapter.BaseRecycleAdapter;
import com.example.WordCFExam.entity.exam.CFExamProfile;

public class CFExamProfileEditableAdapter extends
        BaseRecycleAdapter<BaseEditableAppCompatActivity, CFExamProfile, BaseRecycleAdapter.ItemViewHolder> {

    public CFExamProfileEditableAdapter(BaseEditableAppCompatActivity context) {
        super(context);
    }



}
